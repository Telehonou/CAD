...
if(isset($_GET['go']) || isset($_GET['modulegroupcode']) || isset($_GET['confirme'])) {
	...
	elseif(isset($_GET['modulegroupcode'])) {
		// requête qui récupère les localités un
		$requete = "SELECT * FROM modules WHERE idm = '".$_GET['modulegroupcode']."' ORDER BY mod_modulegroupname ASC";
		// exécution de la requête
		$resultat = $bdd->query($requete) or die(print_r($bdd->errorInfo()));
		// Création de la liste
		while($donnees = $resultat->fetch(PDO::FETCH_ASSOC)) {
