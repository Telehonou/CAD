...
$(document).ready(function() {
	var $modulegroupcode = $('#modulegroupcode');
	...
	$.ajax({
		...
		success: function(json) {
			$.each(json, function(index, value) {
				...
				$('#modulegroupcode').append('<option value="'+ index +'">'+ value +'</option>');
			});
		}
	});

	// à la sélection de la localité un dans la liste
	$modulegroupcode.on('change', function() {
		var val = $(this).val(); // on récupère la valeur de la localité un
		if(val != '') {
			$.ajax({
				url: 'liste.php',
				data: 'modulegroupcode='+ val, // on envoie $_GET['localite_un']
				dataType: 'json',
				success: function(json) {
					$.each(json, function(index, value) {
