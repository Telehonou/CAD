...
<?php
	...
	{
		$id = $_GET['edit_id'];
		$stmt_edit = $DB_con->prepare('SELECT u_name,u_username, u_password, u_rolecode, email, telephone FROM users WHERE u_userid =:uid');
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE users SET u_name=:nom, u_username=:uname, u_password=:pass, u_rolecode=:rol, email=:em, telephone=:tel, created_at=:creat, updated_at=:upda, avatar=:upic WHERE u_userid=:uid');
