...
require '../../../Data/Manager.php';
 ...
 $DB=$man->connexion();
	...
	{
		$stmt_select = $DB_con->prepare('SELECT avatar FROM system_users WHERE u_userid =:uid');
...
<?php
    $sql="SELECT * FROM system_users  ,roles "
                              ."WHERE system_users.u_rolecode=roles.role_rolecode AND system_users.u_userid=".$_SESSION['user_id'];
...
<div class="row">
<?php
	$stmt = $DB_con->prepare('SELECT u_userid, u_username, email, telephone,avatar FROM system_users ORDER BY u_userid DESC');
	$stmt->execute();
if($stmt->rowCount() > 0)
{
